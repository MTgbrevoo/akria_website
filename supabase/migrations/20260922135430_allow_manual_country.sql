-- Allow manually entered countries while retaining all other checkout invariants.
begin;

alter table public.orders drop constraint orders_v1_valid;
alter table public.orders add constraint orders_v1_valid check (
  checkout_version is null or (
    ...
    and country is not null and length(btrim(country)) between 1 and 254 and country !~ '[[:cntrl:]]'
...
begin
  ...
  if p_input->>'country' ~ '[[:cntrl:]]' or p_input->>'email' !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$'
